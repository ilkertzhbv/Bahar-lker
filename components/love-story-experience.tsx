...
export function LoveStoryExperience() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>("[data-parallax-card]").forEach((item) => {
        gsap.to(item, {
          yPercent: -10,
          ease: "none",
          scrollTrigger: {
...
function SceneNavigation() {
  return (
    <nav className="scene-nav" aria-label="Hikaye bölümleri">
      {sceneNavigation.map((item) => (
        <a key={item.id} href={`#${item.id}`} aria-label={item.label}>
          <span />
        </a>
      ))}
    </nav>
...
function HeroSection({
  ...
  return (
    <section
      id="hero"
      ref={heroRef}
      className="scene relative flex min-h-screen items-center justify-center overflow-hidden"
    >
      <motion.div
        ...
        style={{
          ...
          scale,
          backgroundImage:
            "url(https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1800&q=88)"
        }}
      />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,230,220,0.13),rgba(8,7,11,0.16)_30%,rgba(8,7,11,0.94)_78%)]" />
      ...
      >
        <p className="mb-5 rounded-full border border-white/15 bg-white/8 px-4 py-2 text-xs uppercase text-champagne/90 backdrop-blur-xl">
          Premium dijital aşk filmi
        </p>
        <h1 className="font-serif text-6xl leading-none text-pearl drop-shadow-2xl sm:text-8xl lg:text-[9.5rem]">
        ...
        <p className="mt-7 max-w-2xl text-balance text-lg leading-8 text-white/78 sm:text-2xl">
          Bir mesajla başlayan hikayenin sinematik dijital günlüğü.
        </p>
        <div className="mt-8 grid w-full max-w-4xl gap-3 sm:grid-cols-3">
          {openingCredits.map((credit) => (
            <div key={credit.label} className="credit-card">
  ...
  return (
    <section id="message" className="scene message-stage relative grid min-h-screen place-items-center px-5 py-28">
      <div
        ...
        style={{
          backgroundImage:
            "url(https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=1800&q=88)"
        }}
      />
      ...
      <div className="relative z-10 mx-auto max-w-5xl text-center">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-[1fr_0.9fr] md:items-end">
          <div data-cinema-reveal className="message-bubble text-left">
          ...
          <div data-cinema-reveal className="signal-panel">
            <MessageCircleHeart className="text-blush" size={28} />
            <div>
              <span>İlk bildirim</span>
              <strong>Kalbe düşen ilk sahne</strong>
            </div>
          </div>
        </div>
      </div>
...
function BookChapter() {
  return (
    <section id="book" className="scene px-5 py-28">
          ...
          {chapterPages.map((page, index) => (
            ...
            >
              ...
              <div className="p-7">
                ...
                <p className="mt-2 text-sm text-blush/90">{page.subtitle}</p>
...
function MemoryGallery({ onSelect }: { onSelect: (memory: Memory) => void }) {
  return (
    <section id="memories" className="scene bg-[#0c0b10] px-5 py-28">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          ...
          <p data-cinema-reveal className="max-w-md text-white/62">
            Her kare özel bir film posteri gibi açılır. Kendi fotoğrafların eklendiğinde
            bu bölüm tamamen size ait bir premium albüme dönüşür.
          </p>
        </div>
        <div className="mt-12 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-5">
          {memories.map((memory, index) => (
            ...
            >
              ...
              <span className="relative z-10 mt-auto flex flex-col items-start gap-2 p-4 text-left">
                <span className="flex items-center gap-2 text-xs uppercase text-champagne/80">
                  <ImageIcon size={15} />
                  {memory.moment}
                </span>
                <span className="text-base font-semibold text-white">{memory.title}</span>
              </span>
            </motion.button>
          ))}
        </div>
...
function VideoArchive({ onSelect }: { onSelect: (video: VideoItem) => void }) {
  return (
    <section id="videos" className="scene px-5 py-28">
      <div className="mx-auto max-w-7xl">
        <p data-cinema-reveal className="section-kicker">
          Bölüm 4
        </p>
        ...
        <div className="scroll-row mt-12">
          {videos.map((video, index) => (
            ...
            >
              ...
              <span className="absolute left-4 top-4 z-10 rounded-full bg-black/45 px-3 py-1 text-xs uppercase text-champagne backdrop-blur-md">
                {video.tag}
              </span>
...
function MusicChapter() {
  ...
  return (
    <section id="music" className="scene relative overflow-hidden bg-[#07100f] px-5 py-28">
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        ...
        <div data-cinema-reveal className="music-panel">
          {songs.map((song, index) => (
            ...
            >
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-white/10 text-champagne">
                <Music2 size={19} />
              </span>
          ...
          <div className="audio-player">
            <button className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-champagne text-black">
...
function TimelineChapter() {
  return (
    <section id="timeline" className="scene px-5 py-28">
...
function PromiseChapter() {
  return (
    <section className="scene promise-stage px-5 py-28">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <p data-cinema-reveal className="mt-6 max-w-xl text-lg leading-8 text-white/66">
              Bu bölüm hikayeyi sadece geçmişe değil, geleceğe de bağlar. Her madde
              Bahar ve İlker için saklanan küçük bir yıldız gibi.
            </p>
          </div>
          <div className="promise-grid">
            {promises.map((promise, index) => (
              ...
              >
                <Star className="text-champagne" size={20} fill="currentColor" />
...
function LetterChapter() {
  return (
    <section id="letter" className="scene grid min-h-screen place-items-center bg-black px-5 py-28">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,rgba(246,167,184,0.14),transparent_32%),linear-gradient(180deg,rgba(0,0,0,0.95),#000)]" />
...
function FinalChapter() {
  return (
    <section
      id="final"
      className="scene star-scene relative grid min-h-screen place-items-center overflow-hidden px-5 py-28 text-center"
    >
      <div className="stars" />
      <motion.div
        data-cinema-reveal
      ...
      >
        <Heart className="heart-pulse mx-auto mb-8 text-blush" size={58} fill="currentColor" />
        <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/8 px-4 py-2 text-xs uppercase text-champagne backdrop-blur-xl">
          <Film size={15} />
          Final değil, devam sahnesi
        </p>
        ...
        >
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="mt-1 text-sm text-blush/80">{memory.moment}</p>
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 right-5 z-10 flex items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-white/62">{video.tag} · {video.duration}</p>
                </div>
